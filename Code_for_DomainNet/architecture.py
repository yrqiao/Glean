...
class CNN1(nn.Module):
    def __init__(self, conv=nn.Conv2d, linear=nn.Linear):
        super().__init__()
        self.encoder = nn.Sequential(
            conv(3, 32, 3, padding=1), 
            GroupSort(),
            conv(32, 32, 3, padding=1, stride=2), 
            GroupSort(),
            conv(32, 64, 3, padding=1), 
            GroupSort(),
            conv(64, 64, 3, padding=1, stride=2), 
            GroupSort(),
            nn.Flatten(),
        )
...
class CNN(nn.Module):
    def __init__(self, conv=CayleyConv, linear=nn.Linear):
        super().__init__()
        self.encoder = nn.Sequential(
            conv(3, 32, 3), 
            GroupSort(),
            conv(32, 32, 3, stride=2), 
            GroupSort(),
            conv(32, 64, 3), 
            GroupSort(),
            conv(64, 64, 3, stride=2), 
            GroupSort(),
            nn.Flatten(),
        )
        self.classifier = nn.Sequential(
