...
class CayleyConv(StridedConv, nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        self.bias = None
        self.alpha = nn.Parameter(torch.tensor(1.0, dtype=torch.float32, requires_grad=True).cuda())
...
class MLP(nn.Module):
    def __init__(self):
        ...
        lin1 = CayleyLinear(3 * 28 * 28, 128)
        lin2 = CayleyLinear(128, 128)
        lin3 = nn.Linear(128, 2)

        self.encoder = nn.Sequential(lin1, GroupSort(), lin2, GroupSort())
        self.classifier = nn.Sequential(lin3)

    def forward(self, input):
        z = self.get_representation(input)
    ...
    def first_output(self, input):
        input = input.view(input.shape[0], 3 * 28 * 28)
        return self.encoder[0](input)
    
    def second_output(self, input):
        input = input.view(input.shape[0], 128)
        return self.encoder[2](input)
    
class MLP1(nn.Module):
    def __init__(self):
        super(MLP1, self).__init__()
        lin1 = nn.Linear(3 * 28 * 28, 128)
